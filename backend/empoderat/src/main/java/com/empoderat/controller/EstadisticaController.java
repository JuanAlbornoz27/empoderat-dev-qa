...
public class EstadisticaController {
    ...
    @GetMapping("/{tipo}")
    @Operation(summary = "Obtener estadísticas por tipo y periodo", description = "Devuelve las estadísticas del tipo y periodo especificados", security = @SecurityRequirement(name = "jwt"))
    ...
    @GetMapping("/rango")
    @Operation(summary = "Obtener estadísticas por rango de fechas", description = "Devuelve las estadísticas generadas en un rango de fechas", security = @SecurityRequirement(name = "jwt"))
    ...
    @Operation(summary = "Obtener resumen para dashboard", description = "Devuelve un resumen de las principales estadísticas para el dashboard", security = @SecurityRequirement(name = "jwt"))
    public ResponseEntity<Map<String, Object>> obtenerResumenDashboard() {
        Map<String, Object> resumen = new HashMap<>();

        // Obtener estadísticas más recientes de cada tipo
        List<Estadistica> estadisticasUsuarios = estadisticaService.obtenerEstadisticasPorTipoYPeriodo("usuarios",
                "diario");
        List<Estadistica> estadisticasInscripciones = estadisticaService
                .obtenerEstadisticasPorTipoYPeriodo("inscripciones", "diario");
        List<Estadistica> estadisticasCursos = estadisticaService.obtenerEstadisticasPorTipoYPeriodo("cursos",
                "diario");
        List<Estadistica> estadisticasCategorias = estadisticaService.obtenerEstadisticasPorTipoYPeriodo("categorias",
                "diario");
        ...
        if (!estadisticasCategorias.isEmpty()) {
            ...
            resumen.put("inscripcionesPorCategoria",
                    datosCategorias.getOrDefault("inscripcionesPorCategoria", new HashMap<>()));
        }
    ...
    @Operation(summary = "Generar estadísticas para un periodo", description = "Genera todas las estadísticas para el periodo especificado", security = @SecurityRequirement(name = "jwt"))
    ...
    private Map<String, Object> transformarDatosParaFrontend(Estadistica estadistica, String tipo) {
        ...
        switch (tipo) {
            ...
            case "cursos":
                ...
                resultado.put("completadosPorCategoria",
                        datos.getOrDefault("completadosPorCategoria", new HashMap<>()));
                break;
            case "categorias":
                resultado.put("inscripcionesPorCategoria",
                        datos.getOrDefault("inscripcionesPorCategoria", new HashMap<>()));
                resultado.put("tasaCompletadoPorCategoria",
                        datos.getOrDefault("tasaCompletadoPorCategoria", new HashMap<>()));
                break;
        }
        ...
        if (historico != null) {
            if (historico instanceof Map) {
                ...
                if (((Map<?, ?>) historico).keySet().stream()
                        .anyMatch(k -> k instanceof String && ((String) k).contains("-"))) {
                    ...
                    nuevoHistorico.put(hoy.toString(),
                            ((Map<String, Object>) historico).get("nuevosUsuarios") != null
                                    ? ((Number) ((Map<String, Object>) historico).get("nuevosUsuarios")).intValue()
                                    : 0);
